...
def cta_cte_clientes(request, id=None):
    ...
    if form.is_valid() or inicial:
        ...
        try:
            total_debe = (
                cpbs.filter(cpb_tipo__tipo__in=[1, 3, 9]).aggregate(
                    sum=Sum(F("importe_total"), output_field=DecimalField())
                )["sum"]
                or 0
            )
            total_haber = (
                cpbs.filter(cpb_tipo__tipo__in=[2, 4]).aggregate(
                    sum=Sum(F("importe_total"), output_field=DecimalField())
                )["sum"]
                or 0
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            cpbs = cpb_comprobante.objects.filter(
                ...
                fecha_imputacion__lte=form.cleaned_data["fhasta"],
                empresa=empresa,
                estado__in=[1, 2],
            ...
            totales = (
                totales.values(
                    "entidad", "entidad__apellido_y_nombre", "entidad__codigo", "entidad__fact_cuit", "ultimo_pago"
                )
...
def cta_cte_proveedores(request, id=None):
    ...
    if form.is_valid() or inicial:
        ...
        try:
            ...
            total_debe = (
                cpbs.filter(cpb_tipo__tipo__in=[2, 7]).aggregate(
                    sum=Sum(F("importe_total"), output_field=DecimalField())
                )["sum"]
                or 0
            )
    ...
    if ventas_compras == "V":
        if tipo_archivo == "cpbs":
            for c in cpbs:
                ...
                linea += (
                    str(importe_impuestosInt).encode("utf-8").replace(".", "").rjust(15, "0")
                )  # importe_impuestosInt
...
def libro_iva_ventas(request):
    ...
    if form.is_valid():
        ...
        cpbs = (
            cpb_comprobante.objects.filter(
                cpb_tipo__compra_venta="V",
                cpb_tipo__tipo__in=[1, 2, 3, 9, 14, 21, 22, 23],
                empresa=empresa,
        ...
        alicuotas = (
            cpb_comprobante_tot_iva.objects.filter(cpb_comprobante__pk__in=id_cpbs)
            .annotate(
                importe_final=Sum(
                    F("importe_total") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                ),
                importe=Sum(
                    F("importe_base") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                ),
            )
            .select_related("cpb_comprobante", "cpb_comprobante__cpb_tipo", "cpb_comprobante__entidad", "tasa_iva")
    ...
    if form.is_valid():
        ...
        cpbs = (
            ...
            .only(
                ...
                "cpb_tipo__nombre",
                "cpb_tipo__tipo",
                "cpb_tipo__signo_ctacte",
                "cae_vto",
                "cae",
                "entidad__id",
                "entidad__apellido_y_nombre",
...
class caja_diaria(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            cpbs = (
                cpb_comprobante_fp.objects.filter(cpb_comprobante__empresa=empresa)
            ...
            cpbs_detalles = cpbs_detalles.order_by(
                "mdcp_fecha", "cpb_comprobante__pto_vta", "cpb_comprobante__letra", "cpb_comprobante__numero"
            )
...
def ver_cierre_diario(request):
    ...
    cpbs = (
        cpb_comprobante_fp.objects.filter(
            cpb_comprobante__empresa=empresa, mdcp_fecha__gte=fdesde, mdcp_fecha__lte=fhasta
        )
        .filter(Q(cpb_comprobante__estado__in=[1, 2]) | Q(cpb_comprobante__cpb_tipo__tipo=24))
        .order_by("mdcp_fecha", "cpb_comprobante__fecha_cpb")
    )
    ...
    tot_saldo_ini = (
        cpbs.filter(cpb_comprobante__cpb_tipo__tipo=24).aggregate(sum=Sum(F("importe"), output_field=DecimalField()))[
            "sum"
        ]
        or 0
    )
    ...
    variables = RequestContext(
        request, {"tot_debe": tot_debe, "tot_haber": tot_haber, "tot_saldo_ini": tot_saldo_ini, "saldo": saldo}
    )
...
class ingresos_egresos(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            ingresos = cpb_comprobante_fp.objects.filter(
                cpb_comprobante__empresa=empresa,
                mdcp_fecha__gte=fdesde,
                mdcp_fecha__lte=fhasta,
                cpb_comprobante__estado__in=[1, 2],
                cta_ingreso__isnull=False,
                mdcp_salida__isnull=True,
            )
            egresos = cpb_comprobante_fp.objects.filter(
                cpb_comprobante__empresa=empresa,
                mdcp_fecha__gte=fdesde,
                mdcp_fecha__lte=fhasta,
                cpb_comprobante__estado__in=[1, 2],
                cta_egreso__isnull=False,
                mdcp_salida__isnull=True,
            )
            ingresos = ingresos.select_related(
                "cpb_comprobante", "cpb_comprobante__cpb_tipo", "cta_ingreso", "mdcp_banco", "tipo_forma_pago"
            )
            egresos = egresos.select_related(
                "cpb_comprobante", "cpb_comprobante__cpb_tipo", "cta_egreso", "mdcp_banco", "tipo_forma_pago"
            )
            ...
            ingresos_resumen = ingresos.values(
                "tipo_forma_pago__id", "tipo_forma_pago__codigo", "tipo_forma_pago__nombre"
            ...
            egresos_resumen = egresos.values(
                "tipo_forma_pago__id", "tipo_forma_pago__codigo", "tipo_forma_pago__nombre"
            ).annotate(saldo=Sum(ExpressionWrapper(F("importe"), output_field=FloatField())))
            egresos_total = egresos.aggregate(egresos_total=Sum("importe"))

            ingresos_cta_resumen = ingresos.values(
                "cta_ingreso__id", "cta_ingreso__codigo", "cta_ingreso__nombre"
            ).annotate(saldo=Sum(ExpressionWrapper(F("importe"), output_field=FloatField())))
            ingresos_cta_total = ingresos.aggregate(ingresos_cta_total=Sum("importe"))
            egresos_cta_resumen = egresos.values("cta_egreso__id", "cta_egreso__codigo", "cta_egreso__nombre").annotate(
                saldo=Sum(ExpressionWrapper(F("importe"), output_field=FloatField()))
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            for cta in ctas:
                ...
                cpbs = (
                    cpb_comprobante_fp.objects.filter(
                        cpb_comprobante__empresa=empresa, cpb_comprobante__estado__in=[1, 2]
                    )
                    .select_related(
                        "cpb_comprobante", "cpb_comprobante__cpb_tipo", "cta_egreso", "cta_ingreso", "tipo_forma_pago"
                    )
                ...
                debe_ant = (
                    cpbs_anteriores.filter(cta_ingreso=cta).aggregate(
                        sum=Sum(F("importe"), output_field=DecimalField())
                    )["sum"]
                    or 0
                )
                haber_ant = (
                    cpbs_anteriores.filter(cta_egreso=cta).aggregate(
                        sum=Sum(F("importe"), output_field=DecimalField())
                    )["sum"]
                    or 0
                )
                debe_pos = 0
                haber_pos = (
                    cpbs_posteriores.filter(cta_egreso=cta).aggregate(
                        sum=Sum(F("importe"), output_field=DecimalField())
                    )["sum"]
                    or 0
                )
...
class seguimiento_cheques(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cheques = (
            cpb_comprobante_fp.objects.filter(
                cpb_comprobante__empresa=empresa, tipo_forma_pago__cuenta__tipo=2, cpb_comprobante__estado__in=[1, 2]
            )
            .order_by("-fecha_creacion", "-mdcp_fecha")
            .select_related(
                "cpb_comprobante",
                "cta_ingreso",
                "cta_egreso",
                "tipo_forma_pago",
                "mdcp_banco",
                "cpb_comprobante__cpb_tipo",
                "cpb_comprobante__entidad",
                "mdcp_salida__cta_ingreso",
                "mdcp_salida__cpb_comprobante__cpb_tipo",
            )
        )
        ...
        cartera = cheques.filter(
            cta_egreso__isnull=True, cta_ingreso__in=cta_cheques, mdcp_salida__isnull=True
        ).order_by("-mdcp_fecha", "-fecha_creacion")
        cobrados = cheques.filter(
            cpb_comprobante__cpb_tipo__tipo=4,
            mdcp_salida__cpb_comprobante__cpb_tipo__tipo=8,
            cta_egreso__isnull=True,
            cta_ingreso__in=cta_cheques,
            mdcp_salida__isnull=False,
        ).order_by(
            "-fecha_creacion",
            "-mdcp_fecha",
        )
        pagados = cheques.filter(
            cpb_comprobante__cpb_tipo__tipo=7, cta_egreso__in=cta_cheques, mdcp_salida__isnull=True
        ).order_by(
            "-fecha_creacion",
            "-mdcp_fecha",
        )
...
class ProdHistoricoView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            movimientos = cpb_comprobante_detalle.objects.filter(
                cpb_comprobante__estado__in=[1, 2],
...
class RankingsView(VariablesMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if comprobantes:
            ...
            ventas = (
                cpbs.filter(cpb_tipo__compra_venta="V")
                .annotate(
                    pendiente=Sum(F("saldo") * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()),
                    saldado=Sum(
                        (F("importe_total") - F("saldo")) * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()
                    ),
                )
                .order_by(F("m"))
            )
            compras = (
                cpbs.filter(cpb_tipo__compra_venta="C")
                .annotate(
                    pendiente=Sum(F("saldo") * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()),
                    saldado=Sum(
                        (F("importe_total") - F("saldo")) * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()
            ...
            productos_vendidos = (
                productos_vendidos.values("producto__nombre")
                .annotate(
                    tot=Sum(
                        F("importe_total") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                    )
            ...
            productos_comprados = (
                productos_comprados.values("producto__nombre")
                .annotate(
                    tot=Sum(
                        F("importe_total") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                    )
            ...
            ranking_vendedores = (
                comprobantes.values("vendedor__apellido_y_nombre")
                .annotate(tot=Sum(F("importe_total"), output_field=DecimalField()))
                .order_by("-tot")[:10]
            )
            context["ranking_vendedores"] = ranking_vendedores

            ranking_clientes = (
                comprobantes.filter(cpb_tipo__compra_venta="V")
                .values("entidad__apellido_y_nombre")
                .annotate(tot=Sum(F("importe_total") * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()))
                .order_by("-tot")[:10]
            )
            context["ranking_clientes"] = ranking_clientes

            ranking_proveedores = (
                comprobantes.filter(cpb_tipo__compra_venta="C")
                .values("entidad__apellido_y_nombre")
                .annotate(tot=Sum(F("importe_total") * F("cpb_tipo__signo_ctacte"), output_field=DecimalField()))
                .order_by("-tot")[:10]
            )
            ...
            productos_vendidos_categ_total = (
                productos_vendidos_categ.aggregate(
                    sum=Sum(
                        F("importe_total") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                    )
                )["sum"]
            ...
            productos_vendidos_categ = (
                productos_vendidos_categ.values("producto__categoria__nombre")
                .annotate(
                    tot=Sum(
                        F("importe_total") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()
                    )
...
class costo_producto_vendidoView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            movimientos = cpb_comprobante_detalle.objects.filter(
                cpb_comprobante__cpb_tipo__compra_venta="V",
                cpb_comprobante__estado__in=[1, 2],
                cpb_comprobante__cpb_tipo__usa_stock=True,
                cpb_comprobante__empresa=empresa,
                cpb_comprobante__fecha_cpb__gte=fdesde,
...
class comisiones_vendedoresView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo__in=[1, 3, 9]).select_related(
                "cpb_tipo", "estado", "entidad"
            )
            ...
            if porcCom:
                ...
                comprobantes = comprobantes.annotate(total=F(campo)).annotate(
                    comision=Sum(ExpressionWrapper(F(campo) * comision, output_field=FloatField()))
                )
...
def ver_grafico(request):
    productos_vendidos = cpb_comprobante_detalle.objects.filter(
        cpb_comprobante__cpb_tipo__compra_venta="V",
        cpb_comprobante__cpb_tipo__tipo__in=[1, 2, 3, 9, 21, 22, 23],
        cpb_comprobante__estado__in=[1, 2],
        cpb_comprobante__fecha_cpb__year=hoy().year,
    )
    productos_vendidos_total = (
        productos_vendidos.aggregate(
            sum=Sum(F("cantidad") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField())
        )["sum"]
        or 0
    )
    productos_vendidos = (
        productos_vendidos.values("producto__nombre")
        .annotate(tot=Sum(F("cantidad") * F("cpb_comprobante__cpb_tipo__signo_ctacte"), output_field=DecimalField()))
        .order_by("-tot")[:20]
    )
...
def reporte_retenciones_imp(request):
    ...
    if form.is_valid():
        ...
        total = 0
        cpbs = (
            cpb_comprobante_perc_imp.objects.filter(
                cpb_comprobante__empresa=empresa,
                cpb_comprobante__fecha_imputacion__gte=fdesde,
                cpb_comprobante__fecha_imputacion__lte=fhasta,
            )
            .select_related("cpb_comprobante", "perc_imp", "cpb_comprobante__cpb_tipo", "cpb_comprobante__entidad")
            .only("id", "perc_imp", "cpb_comprobante", "importe_total", "detalle")
            .order_by("-cpb_comprobante__fecha_imputacion", "-id")
        )

        rets = (
            cpb_comprobante_retenciones.objects.filter(
                cpb_comprobante__empresa=empresa,
                cpb_comprobante__fecha_imputacion__gte=fdesde,
                cpb_comprobante__fecha_imputacion__lte=fhasta,
            )
        ...
        impuestos = (
            cpb_comprobante.objects.filter(empresa=empresa, fecha_imputacion__gte=fdesde, fecha_imputacion__lte=fhasta)
            .annotate(tot_imp=Sum(F("importe_tasa1") + F("importe_tasa2"), output_field=DecimalField()))
            .filter(tot_imp__gt=0)
            .select_related("cpb_tipo", "entidad")
            .order_by("-fecha_imputacion", "-id")
        )
