        ...
        if form.is_valid():
            ...
            if entidad:
                comprobantes = comprobantes.filter(
                    entidad__apellido_y_nombre__icontains=entidad
                )
            ...
            cpbs = comprobantes.filter(
                fecha_cpb__gte=inicioMesAnt(),
                fecha_cpb__lte=finMes(),
                estado__in=[1, 2],
            )
            if not cpbs.exists():
...
CPBDetalleFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_detalle,
    form=CPBCompraDetalleForm,
    formset=CPBCompraDetalleFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
CPBPIFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_perc_imp,
    form=CPBCompraPercImpForm,
    formset=CPBCompraPIFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
CPBFPFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_fp,
    form=CPBFPForm,
    formset=CPBCompraFPFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
...
class CPBCompraCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        CPBDetalleFormSet.form = staticmethod(
            curry(CPBCompraDetalleForm, request=request)
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            form.is_valid()
            and compras_detalle.is_valid()
            and compras_pi.is_valid()
            and (cpb_fp.is_valid() or (condic_pago == 1))
        ):
    ...
    def form_valid(self, form, compras_detalle, compras_pi, cpb_fp):
        ...
        if cpb_fp and (self.object.condic_pago > 1):
            ...
            op = cpb_comprobante(
                cpb_tipo=tipo_cpb,
                entidad=self.object.entidad,
                pto_vta=self.object.pto_vta,
                letra="X",
                numero=nro,
                fecha_cpb=self.object.fecha_cpb,
                importe_iva=self.object.importe_iva,
                importe_total=self.object.importe_total,
                estado=estado,
            ...
            op.save()
            cobranza = cpb_cobranza(
                cpb_comprobante=op,
                cpb_factura=self.object,
                importe_total=self.object.importe_total,
                desc_rec=0,
            )
            cobranza.save()
...
class CPBCompraEditView(VariablesMixin, SuccessMessageMixin, UpdateView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not puedeEditarCPB(self.get_object()):
            messages.error(
                self.request, u"¡No puede editar un Comprobante con Pagos/Saldado!"
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        form.fields[
            "cliente_categ_fiscal"
        ].initial = self.object.entidad.fact_categFiscal
        ...
        return self.render_to_response(
            self.get_context_data(
                form=form, compras_detalle=compras_detalle, compras_pi=compras_pi
            )
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        CPBPIFormSet.form = staticmethod(curry(CPBCompraPercImpForm, request=request))
        compras_detalle = CPBDetalleFormSet(
            self.request.POST, instance=self.object, prefix="formDetalle"
        )
        compras_pi = CPBPIFormSet(
            self.request.POST, instance=self.object, prefix="formDetallePI"
        )
...
class CPBCompraClonarCreateView(VariablesMixin, CreateView):
    ...
    def get_initial(self):
        ...
        initial["tipo_form"] = "ALTA"
        initial["titulo"] = (
            "Nuevo Comprobante Compras - Clonar CPB: %s" % self.get_object()
        )
        initial["request"] = self.request
        return initial
    ...
    def get(self, request, *args, **kwargs):
                ...
                det.append(
                    {
                        ...
                        "cantidad": c.cantidad,
                        "detalle": c.detalle,
                        "porc_dcto": c.porc_dcto,
                        "tasa_iva": c.tasa_iva,
                        "coef_iva": c.coef_iva,
                        "lista_precios": c.lista_precios,
                        "importe_costo": c.importe_costo,
                        "importe_unitario": c.importe_unitario,
                        "importe_subtotal": c.importe_subtotal,
        ...
        # cpb_fp = CPBFPFormSet(prefix='formFP')
        CPBDetalleFormSet.form = staticmethod(
            curry(CPBCompraDetalleForm, request=request, clonacion=True)
        )
...
def CPBCompraDeleteView(request, id):
    try:
        ...
        if not puedeEliminarCPB(cpb):
            messages.error(
                request, u"¡No puede editar un Comprobante con Pagos/Saldado!"
            )
            return redirect(reverse("cpb_compra_listado"))
        cpb.delete()
...
class CPBPagosViewList(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            empresa = None
        form = ConsultaCpbsCompras(
            self.request.POST or None, empresa=empresa, request=self.request
        )
        comprobantes = (
            cpb_comprobante.objects.filter(
                cpb_tipo__tipo=7, empresa=empresa, estado__in=[1, 2]
            )
        ...
        if form.is_valid():
            ...
            if int(estado) == 1:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=7, empresa=empresa, estado__in=[1, 2, 3]
                ).annotate(cobranzas=Sum("cpb_cobranza_cpb__importe_total"))
            elif int(estado) == 2:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=7, empresa=empresa, estado__in=[3]
                ).annotate(cobranzas=Sum("cpb_cobranza_cpb__importe_total"))
...
PagosFPFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_fp,
    form=CPBFPForm,
    formset=CPBPagosFPFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
PagosCPBFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_cobranza,
    fk_name="cpb_comprobante",
    form=CPBPagoCPBForm,
    formset=CPBPagosCPBFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
PagosRetFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_retenciones,
    form=CPBPagoRetForm,
    formset=CPBPagosRetFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
...
class CPBPagoCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, cpb_fp=cpb_fp, cpb_ret=cpb_ret)
        )
    ...
    def form_valid(self, form, cpb_fp, cpb_ret):
        ...
        self.object.estado = estado
        self.object.letra = "X"
        self.object.numero = ultimoNro(
            12, self.object.pto_vta, self.object.letra, self.object.entidad
        )
...
class CPBPagoEditView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        cpbs_pagos = cpb_cobranza.objects.filter(
            cpb_comprobante=self.object.id, cpb_comprobante__estado__pk__lt=3
        )
        ...
        PagosCPBFormSet = inlineformset_factory(
            ...
            form=CPBPagoCPBForm,
            formset=CPBPagosCPBFormSet,
            extra=len(cpbs_pagos),
            can_delete=False,
            max_num=len(cpbs_pagos),
        )
        d = []
        for cpb in cpbs_pagos:
            ...
            d.append(
                {
                    "detalle_cpb": c.get_cpb_tipo,
                    "desc_rec": "0",
                    "importe_total": cpb.importe_total,
                    "saldo": c.saldo,
                    "id_cpb_factura": c.id,
                    "cpb_factura": c,
                }
        ...
        return self.render_to_response(
            self.get_context_data(form=form, cpb_fp=cpb_fp, cpbs=cpbs, cpb_ret=cpb_ret)
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        PagosFPFormSet.form = staticmethod(curry(CPBFPForm, request=request))
        PagosRetFormSet.form = staticmethod(curry(CPBPagoRetForm, request=request))
        cpb_fp = PagosFPFormSet(
            self.request.POST, instance=self.object, prefix="formFP"
        )
        cpb_ret = PagosRetFormSet(
            self.request.POST, instance=self.object, prefix="formRet"
        )
        cpbs = PagosCPBFormSet(
            self.request.POST, instance=self.object, prefix="formCPB"
        )
        if (
            form.is_valid()
            and cpb_fp.is_valid()
            and cpb_ret.is_valid()
            and cpbs.is_valid()
        ):
...
def CPBPagoDeleteView(request, id):
    try:
        ...
        if not tiene_permiso(request, "cpb_pagos_abm"):
            return redirect(reverse("principal"))
        fps = cpb_comprobante_fp.objects.filter(
            cpb_comprobante=cpb, mdcp_salida__isnull=False
        ).values_list("mdcp_salida", flat=True)
        if len(fps) > 0:
            messages.error(
                request,
                u"¡El Comprobante posee movimientos de cobranza/depósito de Cheques asociados!. Verifique",
            )
            return HttpResponseRedirect(cpb.get_listado())
        else:
            # traigo los fps de los recibos asociados
            pagos = cpb_comprobante_fp.objects.filter(cpb_comprobante=cpb).values_list(
                "id", flat=True
            )
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        if comprobantes.is_valid():
            ...
            for c in comprobantes:
                ...
                d.append(
                    {
                        "detalle_cpb": f["detalle_cpb"],
                        "desc_rec": "0",
                        "id_cpb_factura": f["id_cpb_factura"],
            ...
            response = {
                "status": 0,
                "message": "Verifique que los Totales no superen a los Saldos!",
            }
        ...
        cpbs = cpb_comprobante.objects.filter(id__in=id_cpbs).filter(
            Q(saldo__gt=0, cpb_tipo__id__in=[2, 4, 6, 18])
        )
        ...
        CPBSFormSet = formset_factory(
            CPBSeleccionados, max_num=cant_cpbs, can_delete=False
        )
        comprobantes = CPBSFormSet(prefix="comprobantes", initial=d)
        variables = RequestContext(
            request, {"comprobantes": comprobantes, "total": total}
        )
...
CPBRemitoDetalleFS = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_detalle,
...
class CPBRemitoCCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        CPBRemitoDetalleFS.form = staticmethod(
            curry(CPBRemitoDetalleForm, request=request)
        )
        remito_detalle = CPBRemitoDetalleFS(prefix="formDetalle")
        return self.render_to_response(
            self.get_context_data(form=form, remito_detalle=remito_detalle)
        )
...
class CPBRemitoCEditView(VariablesMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        remito_detalle = CPBRemitoDetalleFS(
            self.request.POST, instance=self.object, prefix="formDetalle"
        )
        if form.is_valid() and remito_detalle.is_valid():
            return self.form_valid(form, remito_detalle)
