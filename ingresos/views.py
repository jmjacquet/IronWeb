...
class CPBSVentasList(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comprobantes = cpb_comprobante.objects.filter(
            cpb_tipo__tipo__in=[1, 2, 3, 9, 14, 21, 22, 23], cpb_tipo__compra_venta="V", empresa=empresa
        ...
        if form.is_valid():
            ...
            cpbs = comprobantes.filter(
                fecha_cpb__gte=inicioMesAnt(), fecha_cpb__lte=finMes(), estado__in=[1, 2]
            )
            if not cpbs.exists():
...
class CPBVentaCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                form=form, ventas_detalle=ventas_detalle, ventas_pi=ventas_pi, cpb_fp=cpb_fp
            )
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            form.is_valid()
            and ventas_detalle.is_valid()
            and ventas_pi.is_valid()
    ...
    def form_valid(self, form, ventas_detalle, ventas_pi, cpb_fp):
        ...
        if cpb_fp and (self.object.condic_pago > 1):
            ...
            recibo = cpb_comprobante(
                ...
                pto_vta=self.object.pto_vta,
                letra="X",
                numero=ultimoNro(7, self.object.pto_vta, "X"),
                id_cpb_padre=self.object,
                fecha_cpb=self.object.fecha_cpb,
                importe_iva=self.object.importe_iva,
                importe_total=self.object.importe_total,
                estado=estado,
...
class CPBVentaNCCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if cpb:
            ...
            form.fields["pto_vta"].initial = cpb.pto_vta
            form.fields["entidad"].initial = cpb.entidad
            form.fields["cpb_tipo"].queryset = cpb_tipo.objects.filter(
                ...
                det.append(
                    {
                        ...
                        "detalle": c.detalle,
                        "porc_dcto": 0,
                        "tasa_iva": c.producto.tasa_iva,
                        "coef_iva": c.producto.tasa_iva.coeficiente,
                        "lista_precios": c.lista_precios,
                        "importe_iva": p["importe_iva"],
                        "origen_destino": c.origen_destino,
                        "importe_subtotal": p["importe_subtotal"],
                        "importe_unitario": p["precio"],
                    ...
                    det.append(
                        {
                            ...
                            "importe_total": c.importe_total,
                            "origen_destino": c.origen_destino,
                            "comprobante_original": int(c.cpb_comprobante.id),
                            "importe_tasa1": c.importe_tasa1,
                            "importe_tasa2": c.importe_tasa2,
                        }
                    )
...
class CPBVentaEditView(VariablesMixin, UpdateView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not puedeEditarCPB(self.get_object()):
            messages.error(
                self.request, u"¡No puede editar un Comprobante Saldado/Facturado o con saldos parciales!"
            )
            return redirect(reverse("cpb_venta_listado"))
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, ventas_detalle=ventas_detalle, ventas_pi=ventas_pi)
        )
...
class CPBRemitoCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, remito_detalle=remito_detalle, cpb=cpb)
        )
...
CPBDetallePresupFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_detalle,
    form=CPBPresupDetalleForm,
    formset=CPBPresupDetalleFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)

CPBDetallePresupLiteFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_detalle,
    form=CPBPresupLiteDetalleForm,
    formset=CPBPresupDetalleFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
...
class CPBPresupViewList(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            if int(estado) == 1:
                comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo=6, empresa=empresa)
            elif int(estado) == 0:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=6, estado__in=[1, 2, 4], empresa=empresa
                )
            elif int(estado) == 2:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=6, estado__in=[3], empresa=empresa
                )
            if fdesde:
                comprobantes = comprobantes.filter(Q(fecha_cpb__gte=fdesde))
            if fhasta:
            ...
            cpbs = cpb_comprobante.objects.filter(
                cpb_tipo__tipo=6, fecha_cpb__gte=inicioMesAnt(), fecha_cpb__lte=finMes(), estado__in=[1, 2, 4]
            )
            if len(cpbs) == 0:
                cpbs = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=6,
...
class CPBPresupEditView(VariablesMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = self.get_form(form_class)
        CPBDetallePresupFormSet.form = staticmethod(curry(CPBPresupDetalleForm, request=request))
        presup_detalle = CPBDetallePresupFormSet(
...
class CPBRecCobranzaViewList(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            if int(estado) == 1:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=4, empresa=empresa, estado__in=[1, 2, 3]
                ).annotate(cobranzas=Sum("cpb_cobranza_cpb__importe_total"))
            elif int(estado) == 2:
                comprobantes = cpb_comprobante.objects.filter(
                    cpb_tipo__tipo=4, empresa=empresa, estado__in=[3]
                ).annotate(cobranzas=Sum("cpb_cobranza_cpb__importe_total"))
...
RecCobranzaFPFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_fp,
    form=CPBRecFPForm,
    formset=CPBRecCobranzaFPFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
RecCobranzaCPBFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_cobranza,
    fk_name="cpb_comprobante",
    form=CPBRecCPBForm,
...
RecCobranzaRetFormSet = inlineformset_factory(
    cpb_comprobante,
    cpb_comprobante_retenciones,
    form=CPBRecRetForm,
    formset=CPBRecRetFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
)
...
def CPBRecCobranzaDeleteView(request, id):
    try:
        ...
        fps = cpb_comprobante_fp.objects.filter(cpb_comprobante=cpb, mdcp_salida__isnull=False).values_list(
            "mdcp_salida", flat=True
        )
...
def CPBCobrosSeleccionarView(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        cpbs = cpb_comprobante.objects.filter(id__in=id_cpbs).filter(
            Q(saldo__gt=0, cpb_tipo__id__in=[1, 3, 5, 14])
        )
...
class CPBLiqProdCreateView(VariablesMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, liqprod_detalle=liqprod_detalle, liqprod_pi=liqprod_pi)
        )
