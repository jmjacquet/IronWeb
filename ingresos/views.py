...
class CPBSVentasList(VariablesMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo__in=[1, 2, 3, 9, 14, 21, 22, 23],
...
class CPBVentaNCCreateView(VariablesMixin,CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if cpb:
            ...
            if cant_det==0:
                cant_det=1
            CPBDetalleFormSet = inlineformset_factory(cpb_comprobante, cpb_comprobante_detalle,form=CPBVentaDetalleForm,fk_name='cpb_comprobante',formset=CPBVentaDetalleFormSet, can_delete=True,extra=0,min_num=cant_det)
            ...
            CPBPIFormSet = inlineformset_factory(cpb_comprobante, cpb_comprobante_perc_imp,form=CPBVentaPercImpForm,fk_name='cpb_comprobante',formset=CPBVentaPIFormSet, can_delete=True,extra=0,min_num=cant_pi)
        else:
            detalles = None       
            perc_imp = None
...
class CPBRemitoViewList(VariablesMixin,ListView):    
    ...
    def get_context_data(self, **kwargs):
        ...
        comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo=5,cpb_tipo__compra_venta='V',estado__in=[1,2],pto_vta__in=pto_vta_habilitados_list(self.request),empresa=empresa).select_related('estado','cpb_tipo','entidad')
        if form.is_valid():                                
            entidad = form.cleaned_data['entidad']                                                              
            fdesde = form.cleaned_data['fdesde']   
            fhasta = form.cleaned_data['fhasta']                                                 
            ...
            if int(estado) == 1:                
                comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo=5,cpb_tipo__compra_venta='V',estado__in=[1,2,3],empresa=empresa).select_related('estado','cpb_tipo','entidad')
            if fdesde:
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():                                
            ...
            if int(estado) == 1:
                ...
                comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo=6, estado__in=[1, 2, 4], empresa=empresa)
            elif int(estado) == 2:
                comprobantes = cpb_comprobante.objects.filter(cpb_tipo__tipo=6, estado__in=[3], empresa=empresa)
            if fdesde:
                comprobantes= comprobantes.filter(Q(fecha_cpb__gte=fdesde))
            if fhasta:
                comprobantes= comprobantes.filter(Q(fecha_cpb__lte=fhasta))  
            ...
            if len(cpbs)==0:
                ...
                empresa=empresa,
                estado__in=[1, 2, 4],
                pto_vta__in=pto_vta_habilitados_list(self.request))[:20]
            comprobantes = cpbs

        context['form'] = form
        context['comprobantes'] = comprobantes.select_related('estado', 'presup_aprobacion', 'cpb_tipo', 'entidad')
        return context
