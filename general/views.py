...
from .forms import EmpresaForm, TareasForm, pto_vta_habilitados_list
from .models import *
...
class VariablesMixin(ContextMixin):
    ...
    def _get_variables_mixin(self, request):
        ...
        context['permisos_ingresos'] = ('cpb_ventas' in permisos_grupo) or ('cpb_cobranzas' in permisos_grupo) or (
                    'cpb_remitos' in permisos_grupo) or ('cpb_presupuestos' in permisos_grupo) or (
                                                   'cpb_liqprod_abm' in permisos_grupo)
        context['permisos_egresos'] = ('cpb_compras' in permisos_grupo) or ('cpb_pagos' in permisos_grupo) or (
                    'cpb_movimientos' in permisos_grupo)
        context['permisos_trabajos'] = ('trab_pedidos' in permisos_grupo) or ('trab_trabajos' in permisos_grupo) or (
                    'trab_colocacion' in permisos_grupo)
        context['permisos_rep_ingr_egr'] = ('rep_cta_cte_clientes' in permisos_grupo) or (
                    'rep_saldos_clientes' in permisos_grupo) or ('rep_cta_cte_prov' in permisos_grupo) or (
                                                       'rep_saldos_prov' in permisos_grupo) or (
                                                       'rep_varios' in permisos_grupo)
        context['permisos_rep_contables'] = ('rep_libro_iva' in permisos_grupo) or (
                    'rep_libro_iva' in permisos_grupo) or ('reporte_retenciones_imp' in permisos_grupo)
        context['permisos_rep_finanzas'] = ('rep_caja_diaria' in permisos_grupo) or (
                    'rep_seguim_cheques' in permisos_grupo) or ('rep_saldos_cuentas' in permisos_grupo)
        context['permisos_entidades'] = ('ent_clientes' in permisos_grupo) or ('ent_proveedores' in permisos_grupo) or (
                    'ent_vendedores' in permisos_grupo)
...
class PrincipalView(VariablesMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comprobantes = cpb_comprobante.objects.filter(estado__in=[1, 2]).filter(
            fecha_cpb__range=[fecha_desde, fecha_hoy], empresa=empresa)

        ventas = comprobantes.filter(cpb_tipo__compra_venta='V', pto_vta__in=pvs,
                                     cpb_tipo__tipo__in=[1, 2, 3, 9, 21, 22, 23])
        total_ventas = \
            ventas.aggregate(sum=Sum(F('importe_total') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))[
                'sum'] or 0
        deuda_cobrar_total = \
            ventas.aggregate(sum=Sum(F('saldo') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))['sum'] or 0
        ventas_mensual = ventas.filter(fecha_cpb__range=[inicioMes(), fecha_hoy])
        total_ventas_mensual = ventas_mensual.aggregate(
            sum=Sum(F('importe_total') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))['sum'] or 0
        deuda_cobrar_mensual = \
        ventas_mensual.aggregate(sum=Sum(F('saldo') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))[
            'sum'] or 0
        ...
        total_compras = \
        compras.aggregate(sum=Sum(F('importe_total') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))[
            'sum'] or 0
        deuda_pagar_total = \
        compras.aggregate(sum=Sum(F('saldo') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))['sum'] or 0
        context['total_compras'] = total_compras
        compras_mensual = compras.filter(fecha_cpb__range=[inicioMes(), fecha_hoy])
        total_compras_mensual = compras_mensual.aggregate(
            sum=Sum(F('importe_total') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))['sum'] or 0
        deuda_pagar_mensual = \
        compras_mensual.aggregate(sum=Sum(F('saldo') * F('cpb_tipo__signo_ctacte'), output_field=DecimalField()))[
            'sum'] or 0
        ...
        context['ultimas_ventas'] = ventas.filter(cpb_tipo__id__in=[1, 3, 5, 14]).order_by('-fecha_cpb',
                                                                                           '-fecha_creacion',
                                                                                           '-id').select_related(
            'entidad', 'cpb_tipo', 'estado')[:10]
        context['ultimas_compras'] = compras.filter(cpb_tipo__id__in=[2, 4, 6, 18], estado__in=[1, 2]).order_by(
            '-fecha_cpb', '-fecha_creacion', '-id').select_related('entidad', 'cpb_tipo', 'estado')[:10]

        if not empresa.pprincipal_aviso_tareas:
            context['tareas'] = None
        elif usr.tipoUsr == 0:
            context['tareas'] = gral_tareas.objects.filter(empresa__id__in=empresas).select_related('usuario_creador',
                                                                                                    'usuario_asignado').order_by(
                '-fecha', '-fecha_creacion', '-id')[:50]
        else:
            context['tareas'] = gral_tareas.objects.filter(empresa__id__in=empresas).filter(
                Q(usuario_asignado=usr) | Q(usuario_asignado__isnull=True)).select_related('usuario_creador',
                                                                                           'usuario_asignado').order_by(
                '-fecha', '-fecha_creacion', '-id')[:50]

        comprobantes_meses = comprobantes.filter(cpb_tipo__tipo__in=[1, 2, 3, 9, 21, 22, 23]).distinct().annotate(
            m=Month('fecha_cpb'), anio=Year('fecha_cpb')).order_by(F('anio'), F('m')).values('m', 'anio')

        meses_cpbs = list(comprobantes_meses.values_list('m', 'anio'))
        ...
        productos_vendidos = cpb_comprobante_detalle.objects.filter(cpb_comprobante__pto_vta__in=pvs,
                                                                    cpb_comprobante__cpb_tipo__compra_venta='V',
                                                                    cpb_comprobante__cpb_tipo__tipo__in=[1, 2, 3, 9, 21,
                                                                                                         22, 23],
                                                                    cpb_comprobante__estado__in=[1, 2],
                                                                    cpb_comprobante__fecha_cpb__range=[fecha_desde,
                                                                                                       fecha_hoy])
        productos_vendidos_total = productos_vendidos.aggregate(
            sum=Sum(F('importe_total') * F('cpb_comprobante__cpb_tipo__signo_ctacte'), output_field=DecimalField()))[
                                       'sum'] or 0
        productos_vendidos = productos_vendidos.values('producto__nombre').annotate(
            tot=Sum(F('importe_total') * F('cpb_comprobante__cpb_tipo__signo_ctacte'),
                    output_field=DecimalField())).order_by('-tot')[:10]
...
class TareasView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            tareas = gral_tareas.objects.filter(empresa__id__in=empresas_habilitadas(self.request)).select_related(
                'usuario_creador', 'usuario_asignado').order_by('-fecha', '-fecha_creacion', '-id')
...
def recargar_clientes(request):
    context = {}
    clientes = egr_entidad.objects.filter(tipo_entidad=1, baja=False,
                                          empresa__id__in=empresas_habilitadas(request)).distinct().order_by(
        'apellido_y_nombre')
...
def recargar_vendedores(request):
    context = {}
    vendedores = egr_entidad.objects.filter(tipo_entidad=3, baja=False,
                                            empresa__id__in=empresas_habilitadas(request)).distinct().order_by(
        'apellido_y_nombre')
...
def recargar_proveedores(request):
    context = {}
    proveedores = egr_entidad.objects.filter(tipo_entidad=2, baja=False,
                                             empresa__id__in=empresas_habilitadas(request)).distinct().order_by(
        'apellido_y_nombre')
...
def recargar_productos(request, tipo):
    context = {}
    productos = prod_productos.objects.filter(baja=False, mostrar_en__in=(tipo, 3),
                                              empresa__id__in=empresas_habilitadas(request)).distinct().order_by(
        'nombre', 'codigo')
