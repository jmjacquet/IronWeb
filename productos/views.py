...
class ProductosView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        productos = (
            prod_productos.objects.filter(empresa__id__in=empresas_habilitadas(self.request), baja=False)
...
prod_precios_FormSet = inlineformset_factory(
    prod_productos,
    prod_producto_lprecios,
    form=Producto_ListaPreciosForm,
    formset=PPFormSet,
    can_delete=True,
    extra=0,
    min_num=1,
...
class ProductosCreateViewModal(VariablesMixin, AjaxCreateView):
    ...
    def form_valid(self, form):
        ...
        ubi_prec = prod_producto_lprecios(
            producto=self.object,
            lista_precios=lista_precios,
            precio_costo=precio_costo,
            precio_cimp=precio_cimp,
...
def prod_precios_actualizar(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        if form.is_valid():
            ...
            cant = actualizar_precios(
                tipo_operacion, tipo_precio, valor, porc, coeficiente, lista, recalcular
            )
        ...
        if cantidad > 0:
            ...
            for p in precios:
                ...
                lista_precios.append(
                    {
                        "codbar": code_bar,
                        "datos": p.producto.codigo_barras,
...
class ProdStockView(VariablesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            productos = prod_producto_ubicac.objects.filter(
                producto__empresa__id__in=empresas_habilitadas(self.request)
...
def prod_stock_nuevo(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            existe_prod_stock = prod_producto_ubicac.objects.filter(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if csv_file.multiple_chunks():
                messages.error(
                    request, "El archivo es demasiado grande (%.2f MB)." % (csv_file.size / (1000 * 1000),)
                )
                return HttpResponseRedirect(reverse("importar_empleados"))
...
def generarCBS(request):
    try:
        prods = prod_productos.objects.exclude(Q(codigo_barras__isnull=True) | Q(codigo_barras__exact=""))
        for p in prods:
...
def prod_consultar_detalles(request):
    ...
    d["producto"] = dict(
        ...
        descr=producto.descripcion,
    )

    if prod_precios:
        d["prod_precios"] = [
            dict(pk=x.pk, lista_precio=x.lista_precios.nombre, precio_venta=x.precio_venta)
            for x in prod_precios
        ]
        ...
        try:
            d["prod_stock"] = [
                dict(
                    ...
                    punto_pedido=x.punto_pedido or 0,
                    stock=x.get_stock_() or 0,
                    unidad=x.producto.get_unidad_display() or "",
                )
                for x in prod_stock
            ]
        except:
...
class ProductosDetalleQRView(VariablesMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["ruta_header"] = "/media/empresa/{}/{}".format(
            settings.ENTIDAD_DIR, "product_detail_header.png"
        )
        context["ruta_footer"] = "/media/empresa/{}/{}".format(
            settings.ENTIDAD_DIR, "product_detail_footer.png"
        )
